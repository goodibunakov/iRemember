...
import nl.joery.animatedbottombar.databinding.ViewTabBinding
...
internal class TabView @JvmOverloads constructor(
    ...
    private val binding by viewBinding(ViewTabBinding::bind)

    private val badgeViews: List<BadgeView> by lazy {
        arrayListOf(
            binding.textBadge,
            binding.iconBadge
        )
    ...
    var icon: Drawable?
        ...
        set(value) {
            ...
            if (newDrawable != null) {
                binding.iconView.setImageDrawable(newDrawable)
            }
        }
    ...
    init {
        ...
        binding.iconBadge.scaleLayout = false
        binding.textBadge.scaleLayout = true
    }
    ...
    private fun updateTabType() {
        animatedView = when (style.selectedTabType) {
            AnimatedBottomBar.TabType.TEXT -> binding.iconLayout
            AnimatedBottomBar.TabType.ICON -> binding.textLayout
        }

        selectedAnimatedView = when (style.selectedTabType) {
            AnimatedBottomBar.TabType.TEXT -> binding.textLayout
            AnimatedBottomBar.TabType.ICON -> binding.iconLayout
        }
    ...
    private fun updateColors() {
        ...
        if (style.selectedTabType == AnimatedBottomBar.TabType.ICON) {
            ...
            binding.textView.setTextColor(tabColor)
        } else if (style.selectedTabType == AnimatedBottomBar.TabType.TEXT) {
            ImageViewCompat.setImageTintList(binding.iconView, ColorStateList.valueOf(tabColor))
            binding.textView.setTextColor(tabColorSelected)
        }
    }
    ...
    private fun updateText() {
        binding.textView.typeface = style.typeface
        binding.textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, style.textSize.toFloat())

        if (style.textAppearance != -1) {
            TextViewCompat.setTextAppearance(binding.textView, style.textAppearance)
        }
    }

    private fun updateIcon() {
        binding.iconView.layoutParams.width = style.iconSize
        binding.iconView.layoutParams.height = style.iconSize
    }
